...
- Mentee sees the offered times and picks one `[R4.4]` → `scheduled`; mentor is notified `[R4.5]`.
...
- After a match, either side can flag "can't make it" `[R5]` → both return to the offer-times step,
  **one re-coordination iteration only** (enforced by a counter on the meeting).
...
- Scheduled jobs (`node-cron`, no external broker):
  - Reminder ahead of the meeting; **both sides confirm arrival** `[R6]`.
