...
**mentee finds a mentor → mentor publishes open time blocks on a calendar → mentee picks one and submits it → mentor approves → meeting is confirmed → it happens → both sides give feedback (nudged every 2 days until they do)**

with automatic WhatsApp/email reminders along the way, and an admin dashboard that shows which meetings are healthy and which ones need a nudge.
